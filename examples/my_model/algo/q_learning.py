...
class DQN(base.ValueNet):
    def __init__(self, sess, name, handle, env, sub_len, memory_size=2**10, batch_size=64, update_every=5, use_kf_act=False, eps_k=0.2):

        super().__init__(sess, name, handle, env, update_every=update_every, use_kf_act=use_kf_act, eps_k=eps_k)
...
class MFQ(base.ValueNet):
    def __init__(self, sess, name, handle, env, sub_len, eps=1.0, update_every=5, memory_size=2**10, batch_size=64):
        super().__init__(sess, name, handle, env, use_mf=True, update_every=update_every)

        config = {
            'max_len': memory_size,
            'batch_size': batch_size,
            'obs_shape': self.feature_space,
    ...
    def train(self):
        ...
        for i in range(batch_name):
            ...
            target_q = self.calc_target_q(obs=obs_next, rewards=rewards, dones=dones, prob=act_prob_next)
            loss, q = super().train(obs=obs, target_q=target_q, prob=act_prob, acts=acts, masks=masks)
            ...
            if i % 50 == 0:
                print('[*] LOSS:', loss, '/ Q:', q)
                # import wandb
                # wandb.log({f'loss/{self.name}': loss})
